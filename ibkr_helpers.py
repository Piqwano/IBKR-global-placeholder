...
def *place_child_bracket_orders(contract: Stock, qty: int, entry_ref_price: float,
trail_pct: float, tp_pct: float,
oca_group_suffix: str = “”) -> Tuple[Optional[Trade], Optional[Trade]]:
ib = get_ib()
oca = f”OCA*{contract.symbol}_{oca_group_suffix or int(_time.time())}”
...
def *place_bracket(contract: Stock, qty: int, entry_price_est: float,
...
oca_group = f”OCA*{contract.symbol}*{parent_id}*{int(_time.time())}”
