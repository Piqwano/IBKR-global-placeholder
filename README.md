...
each redeploy. The gateway image logs prompts to `docker compose logs -f ib-gateway`.
...
1. Place new reduced-qty child bracket (`_place_child_bracket_orders`).
1. Verify it’s live via `_verify_order_live`.
1. ONLY THEN cancel the old parent + old children.
